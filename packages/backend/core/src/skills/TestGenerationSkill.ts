...
import type { ProjectConfig, TestFramework } from '@testmind/shared';
...
export class TestGenerationSkill extends BaseSkill {
  ...
  async execute(context: SkillContext): Promise<SkillResult> {
    ...
    try {
      ...
      return this.failure(`Test generation failed: ${message}`, {
        error: message,
      });
    }
  }
  ...
  private async generateTestForFunction(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.log(`Failed to generate test for ${functionName}: ${message}`);
      return null;
    }
  }
  ...
  private async generateTestForFile(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.log(`Failed to generate test for ${filePath}: ${message}`);
      return null;
    }
  }
