...
type AnalyzerFileData = Awaited<ReturnType<StaticAnalyzer['analyzeFile']>>;
...
export class RefactorSkill extends BaseSkill {
  ...
  async execute(context: SkillContext): Promise<SkillResult> {
    ...
    try {
      ...
      const analysis: RefactorSummary = {
        filesAnalyzed: 0,
        issuesFound: 0,
      ...
      return this.failure(`Refactoring failed: ${message}`, {
        error: message,
      });
    }
  }
  ...
  private async initialize(context: SkillContext): Promise<void> {
    ...
    this.staticAnalyzer = new StaticAnalyzer(projectConfig, new FileCache());
  ...
  private async generateRefactorings(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.log(`Failed to generate refactoring for ${filePath}: ${message}`);
    }
   ...
   */
  private buildRefactorPrompt(filePath: string, analysis: FileAnalysisResult): string {
