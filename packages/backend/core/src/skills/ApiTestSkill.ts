...
export class ApiTestSkill {
  ...
  async execute(context: SkillContext): Promise<SkillResult> {
    ...
    const { input, options } = contextAny;
  ...
  private parseEndpointFromInput(input: string): ApiEndpoint | null {
    ...
    if (simpleMatch) {
      const rawMethod = simpleMatch[1]?.toUpperCase();
      const rawPath = simpleMatch[2]?.trim();
      if (rawMethod && rawPath) {
        return {
          method: rawMethod as ApiEndpoint['method'],
          path: rawPath,
        };
      }
    }
  ...
  private generateRestTestFromEndpoint(
    ...
    const framework = options.framework ?? 'supertest';
    const tests: string[] = [];
  ...
  private async generateTestWithLLM(
    ...
    const llmService = this.resolveLLMService(context);
    if (!llmService?.generate) {
...
`;
    ...
    const response = await llmService.generate({
      provider: contextAny.llmProvider || 'openai',
      model: contextAny.llmModel || 'gpt-4',
      prompt,
      temperature: 0.3,
      maxTokens: 2000,
    });
    ...
    return codeMatch?.[1] ?? fallbackContent;
  }
  ...
  private resolveLLMService(context: SkillContext): LLMService | undefined {
    const contextAny = context as any;
    return this.llmService ?? contextAny?.llmService;
  }
}
...
export function createApiTestSkill(llmService?: LLMService): ApiTestSkill {
  const skill = new ApiTestSkill(llmService);
  if (llmService) {
    (skill as any).llmService = llmService;
  }
  return skill;
}
