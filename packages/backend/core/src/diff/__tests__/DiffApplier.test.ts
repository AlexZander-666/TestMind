...
describe('DiffApplier', () => {
  ...
  describe('applyFileDiff', () => {
    it('should apply simple modification', async () => {
      ...
      // 生成diff
      const diff = generator.generateDiff(testFile, oldContent, newContent);
    ...
    it('should detect conflicts when file content changed', async () => {
      ...
      const diff = generator.generateDiff(testFile, originalContent, expectedContent);
  ...
  describe('applyMultipleDiffs', () => {
    it('should apply multiple diffs successfully', async () => {
      ...
      const diff1 = generator.generateDiff(file1, 'content1', 'modified1');
      const diff2 = generator.generateDiff(file2, 'content2', 'modified2');
    ...
    it('should stop on first failure when allowPartial is false', async () => {
      ...
      const diff1 = generator.generateDiff(file1, 'expected', 'new1');
      const diff2 = generator.generateDiff(file2, '', 'new2');
