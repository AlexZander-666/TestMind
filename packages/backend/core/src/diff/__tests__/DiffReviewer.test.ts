...
describe('DiffReviewer', () => {
  ...
  describe('reviewNonInteractive', () => {
    it('should auto-accept all diffs when specified', async () => {
      const diff1 = generator.generateDiff('file1.ts', 'old1', 'new1');
      const diff2 = generator.generateDiff('file2.ts', 'old2', 'new2');
    ...
    it('should auto-reject all diffs when not auto-accepting', async () => {
      const diff = generator.generateDiff('file.ts', 'old', 'new');
  ...
  describe('reviewWithConfidence', () => {
    it('should auto-accept high confidence diffs', async () => {
      ...
      const highConfidenceDiff = {
        ...generator.generateDiff('file1.ts', 'old', 'new'),
        confidence: 0.95,
      };

      const lowConfidenceDiff = {
        ...generator.generateDiff('file2.ts', 'old', 'new'),
        confidence: 0.7,
      };
  ...
  describe('generateReviewReport', () => {
    it('should generate comprehensive review report', async () => {
      const diff1 = generator.generateDiff('accepted.ts', 'old', 'new');
      const diff2 = generator.generateDiff('rejected.ts', 'old', 'new');
    ...
    it('should calculate acceptance rate correctly', async () => {
      const diffs = [
        generator.generateDiff('f1.ts', '', 'new1'),
        generator.generateDiff('f2.ts', '', 'new2'),
        generator.generateDiff('f3.ts', '', 'new3'),
      ];
  ...
  describe('exportDecisions and applyDecisions', () => {
    ...
    it('should apply previously saved decisions', async () => {
      const diff1 = generator.generateDiff('file1.ts', '', 'new1');
      const diff2 = generator.generateDiff('file2.ts', '', 'new2');
  ...
  describe('edge cases', () => {
    ...
    it('should handle invalid decisions JSON gracefully', async () => {
      const diff = generator.generateDiff('file.ts', '', 'new');
