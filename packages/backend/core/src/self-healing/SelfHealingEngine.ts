...
export enum HealingStrategy {
  ...
  /** 定位器替换 */
  LOCATOR_REPLACEMENT = 'locator_replacement',

  /** 超时延长 */
  TIMEOUT_EXTENSION = 'timeout_extension',

  /** 断言调整 */
  ASSERTION_ADJUSTMENT = 'assertion_adjustment',
...
export class SelfHealingEngine {
  ...
  private readonly config: Required<Omit<SelfHealingConfig, 'llmService'>>;
  ...
  async heal(
    ...
    if (classification.failureType === 'test_fragility' && failure.selector) {
      ...
      if (newLocator && strategy === HealingStrategy.AUTO_FIX) {
        // 自动修复
        healed = await this.attemptAutoFix(failure.selector, newLocator, context.pageContext);
      }
    }
    ...
    suggestions = await this.fixSuggester.suggestFixes(failure, {
      ...
      alternativeSelectors: newLocator
        ? [this.convertLocatorToDescriptor(newLocator)]
        : context.alternativeSelectors ?? [],
    });
  ...
  private async attemptAutoFix(
    ...
    if (!browserContext?.adapter?.applyLocatorPatch) {
      return false;
    }

    try {
      const applyPatch = browserContext.adapter.applyLocatorPatch;
      await applyPatch(selector, locator.metadata?.selector ?? locator.metadata?.xpath ?? '');
      return true;
    } catch (error) {
      logger.warn('Auto-fix patch failed, falling back to suggestions only', { error });
      return false;
    }
  }
  ...
  private async relocateElement(
    ...
    return await this.locatorEngine.locateElement(descriptor, context.pageContext);
  }
  ...
  private async recordIntent(
    ...
    if (!failure.selector || !context.pageContext?.adapter) {
      return undefined;
    }

    try {
      const element = await context.pageContext.adapter.findElement(failure.selector);
      ...
      return await this.intentTracker.recordIntent(
        ...
        element,
        {
          ...
          adapter: context.pageContext.adapter,
        },
      );
    } catch (error) {
      logger.debug('recordIntent skipped due to adapter failure', { error });
      return undefined;
    }
  }
  ...
  private determineStrategy(classification: ClassificationResult): HealingStrategy {
    // 如果是真实Bug，不应该自动修复
    if (classification.failureType === 'real_bug' || classification.failureType === 'environment') {
      return HealingStrategy.CANNOT_FIX;
    }

    if (classification.failureType !== 'test_fragility') {
      return HealingStrategy.CANNOT_FIX;
    }
