...
export class FixSuggester {
    ...
    switch (classification.failureType) {
      case 'test_fragility':
        ...
        if (suggestions.length === 0) {
          suggestions.push(this.createStabilityGuidance());
        }
        break;
  ...
  private suggestSelectorUpdates(
    ...
    const suggestions: FixSuggestion[] = [];
    const currentSelector = context.currentSelector ?? failure.selector;

    if (!context.alternativeSelectors || !currentSelector) {
      return suggestions;
    }
    ...
    for (const altSelector of context.alternativeSelectors) {
      ...
      suggestions.push({
        ...
        description: `Update selector to use ${this.getLocatorStrategyName(
          altSelector,
        )} (prefer data-testid / aria-label based selectors when available)`,
        diff,
  ...
  private suggestRetryLogic(
    ...
    return {
      ...
      description: 'Check network/service health and add retry logic for transient environment issues',
      diff,
      confidence: 0.8,
  ...
  private suggestAssertionFix(
    ...
    return {
      ...
      description: 'Review implementation and update assertion to match confirmed behavior',
      diff,
...
`;
    ...
    try {
      ...
      const type = Object.values(FixType).includes(parsed.type as FixType)
        ? (parsed.type as FixType)
        : FixType.OTHER;

      if (!description || !diff) {
        logger.warn('LLM suggestion missing required fields, skipping entry');
        return null;
      }

      const validEfforts: FixSuggestion['estimatedEffort'][] = ['low', 'medium', 'high'];
      const effort: FixSuggestion['estimatedEffort'] = validEfforts.includes(
        parsed.estimatedEffort as FixSuggestion['estimatedEffort'],
      )
        ? (parsed.estimatedEffort as FixSuggestion['estimatedEffort'])
        : 'medium';
      ...
      return {
        ...
        reasoning: typeof parsed.reasoning === 'string' ? parsed.reasoning : 'LLM generated suggestion',
...
+ expect(value).toBe(${JSON.stringify(actualValue)})`);
  ...
  private createStabilityGuidance(): FixSuggestion {
    return {
      type: FixType.UPDATE_SELECTOR,
      description: 'Use stable selectors (data-testid / aria-label / role) instead of brittle classes or XPath',
      diff: 'N/A',
      confidence: 0.6,
      estimatedEffort: 'low',
      reasoning: 'Selectors tied to structure or styling are fragile. Prefer semantic attributes specifically added for testing.',
      alternativeApproaches: [
        'Add data-testid attributes to critical elements',
        'Leverage aria-label or role-based queries for accessibility-friendly selectors',
      ],
    };
  }
