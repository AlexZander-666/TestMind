...
interface CypressGlobal {
  get(selector: any, options?: any): any;
  $(selector: any): any;
  wait(ms: number): any;
...
export class CypressAdapter implements BrowserAdapter {
  ...
  private readonly cy: CypressGlobal;
  ...
  async click(element: ElementHandle): Promise<void> {
    return new Promise((resolve) => {
      const el = this.unwrapElement(element);
      const $el = this.cy.$(el);
      this.cy.get($el as any).click().then(() => resolve());
    });
  }
  ...
  async fill(element: ElementHandle, value: string): Promise<void> {
    return new Promise((resolve) => {
      const el = this.unwrapElement(element);
      const $el = this.cy.$(el);
      this.cy.get($el as any).clear().type(value).then(() => resolve());
    });
  }
