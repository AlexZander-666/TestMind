...
import type { BrowserAdapter, BrowserContext, ElementHandle } from './adapters/BrowserAdapter';
...
export class IntentTracker {
  ...
  async relocateByIntent(
    intent: TestIntent,
    currentPage?: BrowserContext | any, // 当前页面上下文
  ): Promise<{
    element: any;
    newSelector: string;
    confidence: number;
    ...
    // 策略1: 尝试使用原始选择器
    const element = await this.tryOriginalSelector(intent, currentPage);
    if (element) {
      return {
        element,
  ...
  private async extractElementFeatures(
    ...
    if (adapter) {
      return this.extractWithAdapter(element, adapter, includeVisual);
    }
...
`;
   ...
   * 查询元素（模拟）
   */
  private async queryElement(selector: string, page: BrowserContext | any): Promise<any> {
    if (page?.adapter?.findElement) {
      try {
        return await page.adapter.findElement(selector);
      } catch (error) {
        logger.debug('queryElement adapter failure', { selector, error });
      }
    }
    return null;
  }
   ...
   * 通过XPath查询元素（模拟）
   */
  private async queryElementByXPath(xpath: string, page: BrowserContext | any): Promise<any> {
    if (page?.adapter?.findElement) {
      try {
        return await page.adapter.findElement(xpath);
      } catch (error) {
        logger.debug('queryElementByXPath adapter failure', { xpath, error });
      }
    }
    return null;
  }
  ...
  private generateIntentId(testName: string, action: ActionType, selector: string): string {
    const hashInput = `${testName}:${action}:${selector}`;
  ...
  private async extractWithAdapter(
    ...
    for (const attr of keyAttributes) {
      const value = await this.safeGet(() => adapter.getAttribute(element, attr));
      if (value) {
    ...
    const textContent =
      (await this.safeGet(() => adapter.getTextContent(element))) ??
      (element as any)?.metadata?.textContent;

    const box = await this.safeGet(() => adapter.getBoundingBox(element));
    const computedStyles = includeVisual
      ? await this.safeGet(() => adapter.getComputedStyles(element))
      : null;

    const features: ElementFeatures = {
      textContent: textContent ?? undefined,
      tagName: (element as any)?.tagName?.toLowerCase(),
      classNames: (element as any)?.metadata?.className?.split?.(' ').filter(Boolean),
      attributes,
    };
