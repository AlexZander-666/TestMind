...
export class CssSelectorLocator {
  private readonly config: Required<CssSelectorConfig>;
  ...
  static getSelectorRecommendations(element: any): Array<{
    ...
    if (classes.length > 0) {
      recommendations.push({
        selector: classes.slice(0, 2).map((c: string) => `.${c}`).join(''),
        confidence: 0.75,
        reason: '类名组合，较稳定',
      });
    }
   ...
   * 生成相对稳定的 CSS 选择器
   */
  generateStableSelector(element: any): string {
    if (!element) return '';
