...
export class VisualLocator {
  private readonly config: Required<VisualLocatorConfig>;
  ...
  private extractVisualFeatures(descriptor: ElementDescriptor): VisualFeatures | null {
    const {visualSignature} = descriptor;
  ...
  private calculateColorSimilarity(color1?: string, color2?: string): number {
    ...
    const normalizedColor1 = color1 ?? '';
    const normalizedColor2 = color2 ?? '';

    // 简化实现：精确匹配
    if (normalizedColor1.toLowerCase() === normalizedColor2.toLowerCase()) {
      return 1.0;
    }

    // RGB相似度计算（简化版）
    const rgb1 = this.hexToRgb(normalizedColor1);
    const rgb2 = this.hexToRgb(normalizedColor2);
