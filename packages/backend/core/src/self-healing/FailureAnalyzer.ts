...
export class FailureAnalyzer {
  ...
  async collectFailureContext(
    ...
    try {
      // 尝试收集截图（如果页面对象可用）
      if (pageObj?.screenshot) {
        const screenshot = await pageObj.screenshot({ encoding: 'base64' });
      ...
      if (pageObj?.context) {
        const logs = await this.collectConsoleLogs(pageObj);
        context.consoleErrors = logs;
      }

      // 尝试收集网络日志（Playwright）
      if (pageObj?._networkRecorder) {
        const networkLogs = await this.collectNetworkLogs(pageObj);
