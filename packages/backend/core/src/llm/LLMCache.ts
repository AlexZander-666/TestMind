...
export class LLMCache {
  ...
  private readonly config: Required<Omit<LLMCacheConfig, 'persistencePath'>>;
  ...
  private generateKey(prompt: string, provider?: string, model?: string): string {
    ...
    const hash = crypto
      .createHash('sha256')
      .update(`${normalizedProvider}:${normalizedModel}:${prompt}`)
      .digest('hex');
  ...
  private findSimilarEntry(
    ...
    for (const [key, entry] of this.cache.entries()) {
      ...
      if ((provider && entry.provider !== provider) || (model && entry.model !== model)) {
        continue;
      }
  ...
  private calculatePromptSimilarity(prompt1: string, prompt2: string): number {
    // Tokenize prompts (split by whitespace, punctuation, and camelCase)
    const tokenize = (text: string): Set<string> => {
      const camelSplit = text.replace(/([a-z0-9])([A-Z])/g, '$1 $2');
      const cleaned = camelSplit.replace(/[_\-]/g, ' ').replace(/[^\w\s]/g, ' ');
      ...
      const synonymMap: Record<string, string[]> = {
        ...
        authentication: ['auth', 'login', 'signin', 'authorize'],
        login: ['signin', 'authenticate', 'auth'],
        signin: ['login', 'authenticate', 'auth'],
        test: ['spec', 'case', 'verify', 'validate'],
        tests: ['spec', 'cases', 'verify', 'validate'],
        verify: ['test', 'check'],
      };
      ...
      for (const token of baseTokens) {
        expanded.add(token);
        if (synonymMap[token]) {
          synonymMap[token].forEach(s => expanded.add(s));
        }
      }
    ...
    const containment =
      intersection.size / Math.max(1, Math.min(tokens1.size, tokens2.size));
  ...
  private async saveToDisk(): Promise<void> {
    ...
    try {
      ...
      await fs.writeFile(persistencePath, JSON.stringify(data, null, 2), 'utf-8');
    } catch (error) {
