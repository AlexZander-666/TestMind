...
export class QualityAnalyzer {
  ...
  private analyzeBoundaryConditions(code: string): number {
    ...
    return Math.min(1, coveredBoundaries / 7);
  }
  ...
  private analyzeReadability(code: string): number {
    ...
    const testNames = code.match(/it\(['"](.+?)['"]/g) || [];
    const hasDescriptiveNames = testNames.length > 0 && testNames.every((name) => {
    ...
    if (!hasDescriptiveNames) score -= 0.3;
    ...
    if (commentRatio >= 0.1 && commentRatio <= 0.3) {
      ...
      score += 0.1;
    } else if (commentRatio > 0.5 && !hasAAA) {
    ...
    const hasMagicNumbers = /\b\d{4,}\b/.test(code); // Numbers with 4+ digits
