...
export class PromptBuilder {
  ...
  private determinePromptTier(context: FunctionContext): PromptTier {
    const {cyclomaticComplexity} = context.complexity;
...
`;
    ...
    if (tier.complexity === 'complex') {
      const edgeCaseCount = Array.isArray((context as any)?.strategy?.edgeCases)
        ? (context as any).strategy.edgeCases.length
        : 0;
...
- Consider edge cases (${edgeCaseCount} identified)
`;
    }
...
`;
  ...
  public learnFromError(errorDescription: string): void {
    if (!this.commonErrorCases.includes(errorDescription)) {
      ...
      // Keep only the most recent 10 entries to avoid prompt bloat
      if (this.commonErrorCases.length > 10) {
        this.commonErrorCases = this.commonErrorCases.slice(-10);
      }
    }
  }
