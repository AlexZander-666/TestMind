...
import { TestGenerationError } from '../errors';
...
export class TestGenerator {
  private readonly planner: TestStrategyPlanner;
  private readonly promptBuilder: PromptBuilder;
  private readonly llm: LLMService;
  private readonly logger = createComponentLogger('TestGenerator');
  ...
  async generateUnitTest(
    ...
    try {
      ...
      const prompt = this.promptBuilder.buildUnitTestPrompt({
        context,
        strategy,
        framework, // Use framework from parameter
  ...
  async generateE2ETest(
    ...
    throw new TestGenerationError(
      'E2E test generation is not yet available',
      {
        feature: 'e2e-test-generation',
        status: 'planned',
        workaround: 'Use CLI self-healing with existing E2E suites',
        projectId,
        userStory,
      },
    );
