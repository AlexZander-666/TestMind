...
describe('PromptBuilder - Enhanced Features', () => {
  ...
  describe('Error Case Learning', () => {
    ...
    it('should limit error cases to 10', () => {
      ...
      // 解析“Common Mistakes”段落里的错误项
      const errorLines = prompt.match(/- Error \d+/g) ?? [];
      const errorIds = errorLines.map(line => Number(line.replace('- Error ', '')));

      expect(errorIds.length).toBe(10);
      expect(errorIds).toContain(14);
      expect(errorIds).toContain(5);
      expect(errorIds).not.toContain(4);
      expect(errorIds).not.toContain(3);
    });
  });
