...
export class SmartTestOptimizer {
  ...
  private createBoundaryTest(
    ...
    return {
      ...
      description: `Boundary test ensuring ${functionName} handles ${type} value for ${paramName}`,
      ...
      assertions: [
        {
          type: 'truthy',
          message: `Function ${functionName} should handle ${type} boundary for ${paramName}`,
        },
      ],
      metadata: {
  ...
  private async recommendTestScenarios(context: FunctionContext): Promise<TestScenario[]> {
    ...
    const hasErrorSignals = (context.signature.throws?.length ?? 0) > 0
      || Boolean(context.sideEffects?.some(effect =>
        (effect.description || '').toLowerCase().includes('error'),
      ));
    if (hasErrorSignals) {
      scenarios.push({
        id: generateUUID(),
  ...
  private async enhanceWithSmartMocks(
    ...
    for (const test of tests) {
      ...
      if (context.dependencies && context.dependencies.length > 0) {
        const mockImplementations = await this.generateSmartMocks(context.dependencies);
        enhancedTest.mocks = Object.keys(mockImplementations);
        enhancedTest.metadata = {
          ...enhancedTest.metadata,
          mockImplementations,
        };
      }
   ...
   * 生成智能Mock数据
   */
  private async generateSmartMocks(dependencies: Dependency[]): Promise<Record<string, unknown>> {
    ...
    for (const dependency of dependencies) {
      ...
      } else if (normalized.includes('http') || normalized.includes('api')) {
        mocks[depName] = {
      ...
      } else if (normalized.includes('file') || normalized.includes('fs')) {
        mocks[depName] = {
  ...
  private async estimateCoverage(tests: TestCase[], context: FunctionContext): Promise<number> {
    ...
    const paramsCovered = context.signature.parameters.filter(p =>
      tests.some(t => this.hasInputField(t.input, p.name)),
    );
  ...
  private hasInputField(input: TestCase['input'], field: string): boolean {
    if (!input || typeof input !== 'object') {
      return false;
    }

    if (Array.isArray(input)) {
      return input.some(item => this.hasInputField(item as TestCase['input'], field));
    }
  ...
  private async storeOptimizationPatterns(patterns: any[]): Promise<void> {
    ...
    const chunks: CodeChunk[] = patterns.map((pattern, index) => {
      const failureType = typeof pattern.type === 'string' ? pattern.type : undefined;
      const failureCount =
        typeof pattern.frequency === 'number' ? pattern.frequency : undefined;
      const tags: string[] = ['optimization'];
      if (failureType) {
        tags.push(failureType);
      }
