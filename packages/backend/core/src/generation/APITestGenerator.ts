...
import type { TestSuite, TestCase, Assertion } from '@testmind/shared';
...
type DraftTestCase = Omit<TestCase, 'assertions'> & { assertions?: Assertion[] };
...
export class APITestGenerator {
  ...
  async generateAPITests(config: APITestConfig): Promise<GeneratedAPITest> {
    ...
    const normalizedTests = testCases.map(test => this.normalizeTestCase(test));
    const generatedCode = this.generateFullTestCode(normalizedTests, config);
    const filePath = this.buildTestFilePath(config.baseURL);

    const suite: TestSuite = {
      id: generateUUID(),
      projectId: 'api-tests',
      targetEntityId: `api:${config.baseURL}`,
      testType: 'api',
  ...
  private async generateHappyPathTest(
    ...
    const test = this.createDraftTestCase({
      id: generateUUID(),
  ...
  private async generateValidationTests(
    ...
    if (endpoint.parameters) {
      for (const param of endpoint.parameters.filter(p => p.required)) {
        tests.push(this.createDraftTestCase({
          id: generateUUID(),
...
test('Load test', async () => {
  ...
  private buildTestFilePath(baseURL: string): string {
    const sanitizedUrl = baseURL.replace(/(^\w+:|^)\/\//, '').replace(/[^a-zA-Z0-9]/g, '-');
    const slug = sanitizedUrl || 'api-endpoint';
    return `tests/api/${slug}.test.ts`;
  }

  private generateFullTestCode(testCases: TestCase[], config: APITestConfig): string {
    const sections = [
      this.generateSetupCode(config),
      ...testCases.map(test => test.code || `// TODO: implement ${test.name}`),
      this.generateTeardownCode(config),
    ].filter(Boolean);

    return sections.join('\n\n').trim();
  }

  private createDraftTestCase(test: DraftTestCase): TestCase {
    return {
      ...test,
  ...
  private normalizeTestCase(test: TestCase): TestCase {
    const normalizedAssertions =
      test.assertions && test.assertions.length > 0
        ? test.assertions
        : this.createDefaultAssertions(test);
  ...
  private createDefaultAssertions(test: TestCase): Assertion[] {
    ...
    if (typeof expected.status === 'number') {
      assertions.push({
        ...
        message: `Expected HTTP status ${expected.status}`,
      });
    }
    ...
    if (typeof expected.error === 'string') {
      assertions.push({
        ...
        message: `Expected error message "${expected.error}"`,
      });
    }

    if (typeof expected.sanitized === 'boolean') {
      assertions.push({
        type: expected.sanitized ? 'truthy' : 'falsy',
        actual: 'response.body.sanitized',
        expected: expected.sanitized,
        message: 'Expected sanitized flag to match',
      });
    }
    ...
    if (assertions.length === 0) {
      assertions.push({
        type: 'truthy',
        message: 'API response should be defined',
      });
    }
