...
export class CostTracker {
  ...
  getStats(): {
    ...
    for (const entry of this.costs) {
      ...
      modelStats.tokens += entry.inputTokens + entry.outputTokens;
      byModel[entry.model] = modelStats;

      // 按操作统计
      const opStats = byOperation[entry.operation] ?? {
        count: 0,
        cost: 0,
      };
      opStats.count += 1;
      opStats.cost += entry.cost;
      byOperation[entry.operation] = opStats;
    }
