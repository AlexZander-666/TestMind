...
export class EmbeddingGenerator {
  private readonly config: Required<Omit<EmbeddingConfig, 'apiKey'>> & { apiKey: string };
  ...
  async generateEmbeddings(
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      try {
        ...
        // 批量失败，记录失败项
        logger.error(`[EmbeddingGenerator] Batch ${batchIndex + 1} failed:`, error as Record<string, unknown>);
  ...
  async generateIncremental(
    ...
    const chunksToGenerate = allChunks.filter(chunk => {
      ...
      return existing.content.length !== chunk.content.length ||
