...
import { ContextAnalysisError, NotFoundError, TestMindError } from '../errors';
...
export class ContextEngine {
  ...
  private readonly logger = createComponentLogger('ContextEngine');
   ...
   */
  private async incrementalUpdate(_projectPath: string, changedFiles: string[]): Promise<IndexResult> {
    const startTime = Date.now();
    ...
    const updatedByPath = new Map(updatedFiles.map(file => [file.filePath, file]));

    // Update dependency graph for changed files
    for (const filePath of changedFiles) {
      await this.dependencyBuilder.updateFile(filePath, updatedByPath.get(filePath));
    }
  ...
  async getModuleContext(modulePath: string): Promise<ModuleContext> {
    ...
    for (const func of moduleData.astData.functions.filter((f: FunctionNode) => f.isExported)) {
      try {
  ...
  async updateFile(filePath: string): Promise<void> {
    ...
    const analyzed = await this.staticAnalyzer.analyzeFile(filePath);
    await this.dependencyBuilder.updateFile(filePath, analyzed);
    await this.semanticIndexer.updateFile(filePath);
  }
