...
export class StaticAnalyzer {
  ...
  private readonly logger = createComponentLogger('StaticAnalyzer');

  constructor(private readonly config: ProjectConfig, fileCache?: FileCache) {
  ...
  private _getLineColumn(text: string, index: number): { line: number; column: number } {
    const lines = text.substring(0, Math.max(index, 0)).split('\n');
