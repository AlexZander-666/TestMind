...
export class HybridSearchEngine {
  private readonly vectorStore: EnhancedVectorStore;
  private readonly dependencyGraph: DependencyGraphBuilder;
  private readonly keywordIndex: Map<string, Set<string>> = new Map(); // keyword -> chunk IDs
  private readonly chunkCache: Map<string, CodeChunk> = new Map();
  ...
  private async vectorSearch(
    ...
    try {
      ...
      logger.error('[HybridSearch] Vector search failed:', error as Record<string, unknown>);
      return [];
    }
  }
  ...
  private async dependencySearch(
    ...
    try {
      ...
      const relatedFiles = await this.dependencyGraph.getRelatedFiles(query.filePath, {
        includeReverse: true,
        maxDepth: 2,
        limit: k * 3,
      });
      ...
      for (const relatedFile of relatedFiles) {
        const chunksInFile = Array.from(this.chunkCache.values()).filter(
          chunk => chunk.filePath === relatedFile.path,
        );

        for (const chunk of chunksInFile) {
          const score = relatedFile.weight || 0.5;
          results.push({
            chunk,
            score,
            relevance: score,
      ...
      logger.error('[HybridSearch] Dependency search failed:', error as Record<string, unknown>);
      return [];
    }
  }
